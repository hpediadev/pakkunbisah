                        ...
                        <table  class="display table table-bordered table-striped" id="dynamic-table">
                            ...
                            while($row = $query->fetch_array()){?>
                                ...
                                <td><?php switch($row['sandbox'])
                                    {
                                        case 1:
                                            ?><span class="label label-danger"><a style="color:white" href="?c=<?php echo base64_encode('deactivatesandbox|'.$row['id']);?>">ACTIVE</a></span><?php
                                            break;
                                        case 0:
                                            ?><span class="label label-success"><a style="color:white" href="?c=<?php echo base64_encode('activatesandbox|'.$row['id']);?>">INACTIVE</a></span><?php
                                            break;
                                        default:
                                            ?><span class="label label-default"><a style="color:white" href="?c=<?php echo base64_encode('activatesandbox|'.$row['id']);?>">UNKNOWN</a></span><?php
                                            break;
                                    }?></td>
                                <td><?php echo $row['trialtime']?> days</td>
                                <td><a href="editproduct.php?d=<?php echo base64_encode($row['id']);?>" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></a>   <a href="?c=<?php echo base64_encode('delete|'.$row['id']);?>" class="btn btn-primary btn-xs"><i class="fa fa-ban"></i></a></td>
                                </tr><?php }?>
                            </tbody>
                            <tfoot>
                            <tr>
